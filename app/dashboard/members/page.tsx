...
export default async function MembersPage({ searchParams }: { searchParams: Promise<{ status?: string; memberId?: string }> }) {
  ...
  return (
    <SidebarProvider>
      <AppSidebar side="right" userRole={user.role} studyhallName={user.studyhall.name} activePath="/dashboard/members" />
      <SidebarInset>
        <header className="flex h-16 items-center gap-2 border-b px-4"><SidebarTrigger /><Separator orientation="vertical" className="h-4" /><span className="font-medium">آرشیو اعضا</span></header>
        <main className="grid gap-6 p-4 md:grid-cols-[0.9fr_1.1fr] md:p-6">
          <Card>
            <CardHeader>

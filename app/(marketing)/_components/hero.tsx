  ...
  return (
    <section className="relative min-h-[calc(100vh-4rem)] w-full overflow-hidden border-b border-accent bg-background">
      <div className="absolute inset-0 -z-10 overflow-hidden">
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-linear-to-br from-primary/10 via-transparent to-transparent rounded-full blur-3xl" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-linear-to-tl from-primary/5 via-transparent to-transparent rounded-full blur-3xl" />
      </div>
      <div className="mx-auto flex w-full max-w-(--breakpoint-xl) flex-col items-center justify-between gap-x-10 gap-y-14 px-6 py-12 lg:min-h-[calc(100vh-4rem)] lg:flex-row lg:py-0">
        <div className="max-w-xl text-right">
          ...
          <div className="mt-10 flex flex-col sm:flex-row flex-wrap gap-3 text-sm text-muted-foreground">
          <Bubble>
            <BubbleContent>
              حذف دوباره‌رزروی صندلی
            </BubbleContent>
            <BubbleReactions>
              <span>👍</span>
            </BubbleReactions>
        ...
        <div className="relative w-full lg:max-w-2xl">
          {/* Subtle depth effect */}
          <div className="absolute inset-0 -z-10 bg-linear-to-b from-primary/5 to-transparent rounded-[2rem] blur-2xl" />
          <DashboardPreview />
        </div>
      </div>
    </section>

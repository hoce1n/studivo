...
// v2 shape: seats belong to `Section` and have `number: string`
type Seat = {
  number: string; // corresponds to Seat.number in Prisma schema
  state: SeatState;
  label?: string;
};

const initialSeats: Seat[] = [
  { number: "1", state: "reserved", label: "رزرو فعال" },
  { number: "2", state: "available" },
  { number: "3", state: "available" },
  { number: "4", state: "reserved", label: "مشغول" },
  { number: "5", state: "available" },
  { number: "6", state: "reserved", label: "مشغول" },
  { number: "7", state: "available" },
  { number: "8", state: "available" },
  { number: "9", state: "reserved", label: "رزرو فعال" },
  { number: "10", state: "available" },
  { number: "11", state: "reserved", label: "مشغول" },
  { number: "12", state: "available" },
];
...
export function InteractiveSimulator() {
  ...
  function toggleSeat(seat: Seat) {
    setSeats((currentSeats) =>
      currentSeats.map((currentSeat) => {
        if (currentSeat.number !== seat.number) return currentSeat;

        const nextState =
          currentSeat.state === "available" ? "reserved" : "available";
    ...
    setActiveSeatNumber(seat.state === "available" ? seat.number : null);
  }
  ...
  return (
    ...
    >
      <div className="absolute inset-x-6 top-0 h-px bg-linear-to-l from-transparent via-primary/40 to-transparent" />
        ...
        <div className="space-y-3 lg:max-w-sm">
          ...
          <div>
            ...
            <p className="mt-3 leading-7 text-muted-foreground">
              این مینی‌سالن نشان می‌دهد مدیر سالن مطالعه، کتابخانه یا پانسیون
              کنکور چگونه با یک کلیک وضعیت صندلی و اولین موعد تمدید شهریه را
              شفاف می‌کند.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-emerald-900 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-200">
              ...
              <p className="mt-1 text-2xl font-black">
                {persianSeatNumber.format(availableSeats)}
              </p>
            </div>
            <div className="rounded-2xl border border-red-200 bg-red-50 p-3 text-red-900 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-200">
              ...
              <p className="mt-1 text-2xl font-black">
                {persianSeatNumber.format(reservedSeats)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="relative w-full lg:max-w-xl">
              ...
              <div className="rounded-2xl border border-emerald-200 bg-background/95 p-4 shadow-2xl shadow-emerald-950/10 backdrop-blur dark:border-emerald-500/30">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-sm font-bold leading-6">
                    صندلی با موفقیت به نام دانش‌آموز جدید رزرو شد! موعد اولین
                    تمدید شهریه: ۳۰ روز آینده.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="rounded-[1.5rem] border bg-background/80 p-3 shadow-inner md:p-4">
            ...
            <div
              ...
              role="list"
              aria-label="چیدمان صندلی‌ها"
            >
              {seats.map((seat) => {
                const isAvailable = seat.state === "available";
                const isActive = seat.number === activeSeatNumber;
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive &&
                        "-translate-y-1 ring-2 ring-emerald-400 ring-offset-2 ring-offset-background",
                    )}
                    aria-pressed={!isAvailable}
                    aria-label={`صندلی ${persianSeatNumber.format(Number(seat.number))} ${isAvailable ? "خالی" : "مشغول"}`}
                  >
                    <span
                      className={cn(
                    ...
                    <span className="mt-2 block text-xs font-black sm:text-sm">
                      صندلی {persianSeatNumber.format(Number(seat.number))}
                    </span>

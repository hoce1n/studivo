...
export async function fetchFinanceDashboard(startDate: Date, endDate: Date): Promise<ActionResult<FinanceDashboardData>> {
  ...
  try {
    const [
      ...
      prisma.payment.findMany({
        where: { membership: { studyHallId }, status: "COMPLETED", paidAt: { gte: startDate, lte: endDate } },
        include: { membership: { include: { user: { select: { name: true } } } } }
      }),
      prisma.expense.findMany({
        where: { studyHallId, voidedAt: null, occurredAt: { gte: startDate, lte: endDate } },
      ...
      prisma.membership.findMany({
        where: { studyHallId, status: "ACTIVE", endsAt: { gte: new Date(), lte: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) } },
    ...
    const averageRevenuePerMember = activeMembershipsCount > 0 
      ? activeMemberships.reduce((sum, m) => sum + Number(m.planPrice), 0) / activeMembershipsCount 
      : 0;
    ...
    payments.forEach(p => {
      ...
      trendMap.set(d, { ...current, revenue: current.revenue + Number(p.amount) });
    });
    expenses.forEach(e => {
      const d = e.occurredAt.toISOString().split('T')[0];
      const current = trendMap.get(d) || { revenue: 0, expense: 0 };
      trendMap.set(d, { ...current, expense: current.expense + Number(e.amount) });
    });
    ...
    const expenseCategoryData = Array.from(expenseCatMap.entries()).map(([name, value]) => ({ name, value }));
    ...
    const recentTransactions = [
      ...payments.map(p => ({ id: p.id, type: 'PAYMENT', amount: Number(p.amount), date: p.paidAt, title: p.membership.user.name })),
      ...expenses.map(e => ({ id: e.id, type: 'EXPENSE', amount: Number(e.amount), date: e.occurredAt, title: e.title }))
